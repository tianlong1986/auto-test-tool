...
if [ $POWER = "DC" ];then
##########low level suspend test begin
echo "=================================================="
LEVEL=`upower -d | grep percentage|awk '{print $2}'|awk -F. '{print $1}'`
echo "Begin low battery level suspend test ($((LEVEL-1))%) ..."
LEVEL=`upower -d | grep percentage|awk '{print $2}'|awk -F. '{print $1}'`
click_level low
...
do
	if [ ! -e /var/log/pm-suspend.log ];then
		LEVEL_LOW=`gsettings get org.gnome.settings-daemon.plugins.power percentage-low`
		LEVEL=`upower -d | grep percentage|awk '{print $2}'|awk -F. '{print $1}'`
		if [ $LEVEL -lt $LEVEL_LOW ];then
			echo "Fail to action in low level"
			echo "[item_$1]=F" >> $RESULT
			echo "[item_$1_comment]=Fail to suspend in low level ($((LEVEL-1))%)" >> $RESULT
			break
		fi
		set_wakealarm "+$WAKETIME"
			...
			echo "Low battery level suspend test successful"
			echo "[item_$1]=P" >> $RESULT
			echo "[item_$1_comment]=Low battery level ($((LEVEL-1))%) suspend test successful" >> $RESULT
			break
		fi
	fi
	sleep 1
done
##########low level suspend test end
fi
click_level low 10
click_level critical 5
